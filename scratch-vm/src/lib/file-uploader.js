...
import convertAudioToWav from './movie-audio-converter';
 ...
 */
const soundUpload = function (fileData, fileType, storage, handleSound, handleError, audioContext) {
    let soundFormat;
    switch (fileType) {
    case 'audio/mp3':
    case 'audio/mpeg': {
    ...
    default:
        convertAudioToWav(fileData, audioContext)
            .then(fixed => {
                soundUpload(fixed, 'audio/wav', storage, handleSound, handleError, audioContext);
            })
            .catch(handleError);
        return;
    }
